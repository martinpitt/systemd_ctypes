/*
 * Compile with: gcc -o inotify_cancel_fd inotify_cancel_fd.c -lsystemd && ./inotify_cancel_fd
 */
...
int main() {
    ...
    puts("Setting up inotify sources using dirfds...");
    ...
    ret = sd_event_add_inotify_fd (event_loop, &i_top, work_fd,
    ...
    puts("Added inotify source");

    // Create second source
    int work_fd2 = open("/tmp/work", O_RDONLY);

    ret = sd_event_add_inotify_fd(event_loop, &i_sub, work_fd2,
    ...
    puts("Added second inotify source");
