/*
 * Reproduction using /proc/self/fd/ paths like the Python wrapper
 * Compile with: gcc -o inotify_cancel_procfd inotify_cancel_procfd.c -lsystemd
 */
...
int main() {
    ...
    puts("Setting up inotify sources using /proc/self/fd/ paths...");

    // Open the directory and use /proc/self/fd/ path like Python wrapper
    int work_fd = open("/tmp/work", O_RDONLY);
    if (work_fd < 0) {
        fprintf(stderr, "Failed to open /tmp/work: %m\n");
        return 1;
    }

    char proc_path[256];
    snprintf(proc_path, sizeof(proc_path), "/proc/self/fd/%d", work_fd);

    // Create inotify source using /proc/self/fd/ path
    ret = sd_event_add_inotify(event_loop, &i_top, proc_path,
    ...
    printf("Added inotify source for %s\n", proc_path);

    // Create second source on non-existing path
    int work_fd2 = open("/tmp/work", O_RDONLY);
    char proc_path2[256];
    snprintf(proc_path2, sizeof(proc_path2), "/proc/self/fd/%d", work_fd2);
    
    ret = sd_event_add_inotify(event_loop, &i_sub, proc_path2,
                               IN_CREATE | IN_DELETE | IN_MOVE | IN_DELETE_SELF | IN_IGNORED,
                               inotify_handler, NULL);
    if (ret < 0) {
        fprintf(stderr, "Failed to add sub inotify source: %s\n", strerror(-ret));
        return 1;
    }
    printf("Added second inotify source for %s\n", proc_path2);
